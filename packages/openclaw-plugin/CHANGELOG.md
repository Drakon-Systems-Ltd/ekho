...
### Fixed
- **OpenClaw 2026.8.1 can load the plugin again.** The self-contained ESM bundle no longer inlines a top-level CommonJS `require("node:crypto")`; it consumes the SDK's published ESM entry and refuses builds that contain esbuild's unsupported dynamic-require shim.

## [0.4.7] - 2026-08-29

### Fixed
- **Unsigned/invalid operator-key revocation warnings no longer storm the log.** `noteAdvisoryClaim` used to emit one warning per key per inbox poll. Identical unsigned claims now emit one aggregate warning when the advisory set first appears or changes, stay quiet on identical repeats, and emit a reminder every 120 polls (~10 minutes at the default 5s interval). A process-local 64-scope cap may evict an older identity+fleet entry and re-emit that scope's first warning; it cannot suppress another identity or fleet. Clearing the set resets the throttle so a later recurrence is logged again. Warnings list a bounded, control-character-safe key sample with an omitted count; the throttle fingerprints the complete sorted set using canonical JSON. State is process-local and scoped by the identity public key + fleet. Trust is unchanged: unsigned/invalid revocation remains advisory, blocks new adoption for that poll, and never unpins or tombstones.

## [0.4.6] - 2026-08-27

### Notes
- No OpenClaw plugin surface change. Version bumped in lockstep with the relay and SDK; no agent protocol break.
...
### Added
- **The plugin can now say which bundle it is running (#33).** `package.json` and `openclaw.plugin.json` both asserted a version; `dist/index.js` — the code that actually runs — asserted nothing, and `grep -oE '"?version"?\s*[:=]\s*"0\.[0-9.]+"' dist/index.js` returned zero matches on two live boxes. On this fleet `dist` is replaced independently of `package.json` as routine practice (six and nine `dist/index.js.bak-*` files beside an untouched `package.json`), so a box could report a release version while running different code, and no local check could contradict it — which is how a `0.4.1` with no `revokedOperatorKeys` ledger was read as "0.4.1 spans the fix" when in fact every genuine 0.4.1 contains it. `build.mjs` now injects `__EKHO_BUILD__ = { version, commit, builtAt, artifactSha256 }` as an esbuild banner, and the plugin logs it once at startup as `[ekho-build] …` plus a compact `build=` field on the existing `[ekho-autoreply] listening for inbound (poll …)` line, so one `journalctl | grep` answers it. It is also on `ekho_inbox` as `build`, for an agent with no journal access.

  **The digest is the part that matters.** A stamp alone reproduces the original defect one layer in: hand-patch `dist` and the stamp sits inside the patched bundle still asserting the release it no longer is. So the build hashes the emitted bundle with the digest slot zeroed, writes that digest into the slot, and the plugin re-zeros and re-hashes its own bytes at startup. A patched bundle reports `MODIFIED` and a `running_sha256` that differs from `built_sha256` — verified against a real artifact edited in place, same byte length, one letter changed. Two differently-patched boxes get different `running_sha256` values, so they are distinguishable rather than both merely "not the release". This is tamper-*evidence* against routine hot-patching, not forgery-proofing: anyone who knows the scheme can re-zero the slot and recompute. That needs a signature over a key the box does not hold, which is separate work.
