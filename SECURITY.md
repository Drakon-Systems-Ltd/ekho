...
- **If the console is reachable beyond your private network, set `EKHO_OPERATOR_REQUIRE_TAILNET=1`** (optionally with `EKHO_OPERATOR_TAILNET_USER`). It rejects operator requests that carry no Tailscale identity *before* credentials are processed, so a published relay does not expose its login to the open internet
