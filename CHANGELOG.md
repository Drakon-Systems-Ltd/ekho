...
### Fixed
- **Unsigned/invalid operator-key revocation warnings no longer storm the log.** The OpenClaw plugin used to emit one warning per advisory key on every inbox poll (~5s). Identical unsigned claims now emit one aggregate warning when the set first appears or changes, stay quiet on identical repeats, and emit a reminder every 120 polls (~10 minutes at the default 5s interval). A process-local 64-scope cap may evict an older identity+fleet entry and re-emit that scope's first warning; it cannot suppress another identity or fleet. Clearing the set resets the throttle so a later recurrence is logged again. Warnings list a bounded key sample with an omitted count; the throttle fingerprints the complete sorted set. State is process-local and scoped by the identity public key + fleet. Trust is unchanged: unsigned/invalid revocation remains advisory, blocks new adoption for that poll, and never unpins or tombstones.
...
- **The tailnet operator gate verifies the peer, not just the header (#60).** `EKHO_OPERATOR_REQUIRE_TAILNET=1` is documented as rejecting operator requests that carry no Tailscale identity before credentials are processed, but `tailnetLoginFromHeaders` believed `Tailscale-User-Login` / `Tailscale-User-Name` from any client — and Docker Compose and Helm bind `EKHO_HOST=0.0.0.0`, so anything that could reach the port could set the header and pass the gate on `POST /v1/operator/login` and on every `requireOperatorAuth` route. The identity is now only accepted when the raw socket peer is one of the configured trusted proxies (`EKHO_TRUSTED_PROXY_IPS`, the same list the login throttle uses, default loopback — where `tailscale serve` terminates); a direct connection fails the gate before the body is parsed, no matter what headers it carries. Peer comparison normalises IPv4-mapped addresses, so Serve on a dual-stack loopback listener keeps working, and an absent or unparseable peer address is untrusted. Both call sites go through one `evaluateRequestTailnetGate` helper and `evaluateTailnetGate` now requires an explicit `peerTrusted` argument, so the check cannot be dropped from one path silently.
- **A2A sends clear the same admission gate as `POST /v1/messages` (#59).** `message/send` and `message/stream` reached `createMessage` directly after nothing more than authentication — and `requireAgentAuth` authenticates quarantined and paused agents — so switching transport from `/v1/messages` to `/a2a` bought a sender an exemption from quarantine, rate limits, the policy engine and extension hooks, while `docs/a2a.md` promised the opposite. Both paths now call one shared gate (`src/message-gate.ts`) before any task or message row exists; on A2A the denials map to JSON-RPC `-32050` (quarantined/paused), `-32051` (rate limit), `-32052` (policy) and `-32053` (extension), and `message/stream` returns the error instead of opening an SSE stream.

### Fixed
- **Helm's post-install bootstrap command now targets the real setup entrypoint (#56).** `NOTES.txt` invoked a removed `create-operator.ts`; it now invokes `packages/relay/src/setup.ts`, which is present in the runtime image.
- **Helm's default image tag now resolves to a published image (#57).** The chart no longer defaults to the missing `0.4.5` image. Release `0.4.6` restores the normal lockstep pin across the chart and published GHCR image.
...
### Fixed
- **Second-device operator enrolment works again on the Security console.** After the 16 Aug lockout fixes, `Generate identity` refused whenever *this browser* held no live key — including a clean phone with no seed at all while the laptop already held the fleet trust root (`X6Nv…`). That over-corrected: an empty browser is the normal second-device path (mint here, Endorse from the root device in panel ②), not a stranded revoked seed. The console now allows Generate on an empty browser when a live root exists elsewhere, tells you which device must Endorse next, and still blocks Generate only when this browser still holds a dead/unlocked seed (Forget device first). Live incident 17 Aug 2026: phone setup hit a permanent red wall with the exact false copy *"could never be endorsed"* while the laptop was SIGNING as the root.

### Notes
- Relay/console hot path. OpenClaw/Hermes agent plugins are version-bumped in lockstep; no agent protocol break.
- `2T8zn…` cycle / revoke behaviour is unchanged and correct — do not try to re-endorse a revoked parent from its child key.
...
- **`GET /v1/inbox` no longer 500s on a fractional `?limit=` or returns the whole mailbox on a negative one (#35).** This is the endpoint every agent polls every 5 seconds. `?limit=1.5` is finite, so it reached SQLite as a REAL `LIMIT` and threw `SQLITE_MISMATCH`; `?limit=-5` is also finite, and SQLite treats a negative `LIMIT` as unbounded. `/v1/sent` and `/v1/conversations` already used `clampLimit` (`Math.trunc` + floor 1 + cap 100); inbox was the leftover. Handler landed in #36; tests below prove the default and the cap rather than only "did not 500".
...
### Added
- **An outbound send now names the session that produced it (#17).** An agent identity is per-box: every concurrent session on a host signs with the same key. #32 made the relay return sender metadata verbatim on `GET /v1/sent`, but with only `ekho_origin` on the wire a sibling session still could not tell *"I said that"* from *"someone else has my key"*. Both plugins now stamp `origin_session_id` beside `ekho_origin` — **only** when the host actually supplies a session identity, because minting one would assert that a different session sent every message, which is worse than saying nothing. OpenClaw: `ekho_send` moves from `execute` to `factory`, because the host calls `execute(params, config, { api, signal, toolCallId, onUpdate })` — no session anywhere in it — while `sessionKey`/`sessionId` live on the `OpenClawPluginToolContext` only a `factory` receives (read from installed openclaw 2026.7.1-2). It stamps the stable `sessionKey`, falling back to `sessionId` only if that is all a host exposes, since `sessionId` is regenerated on `/new` and `/reset` and would split one session into many. The other tools are untouched. Hermes: `build_send_input` takes an optional `origin_session_id` and `_handle_ekho_send` feature-detects one from its call kwargs — but no Hermes runtime was available to read the real tool-handler call shape from, so on today's boxes the field is simply absent until that is confirmed. A missing stamp is honest; a fake one is a lie. Plugin freeze: PR only.
